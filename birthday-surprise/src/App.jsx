/**
 * Birthday Surprise — No Overflow Version
 * Every screen is position:fixed, fills viewport exactly.
 * All sizes use vh-based clamp() so nothing scrolls.
 */
...
const Polaroid = ({ src, page }) => {
  const grads = [
    "linear-gradient(135deg,#e8d5f5,#ffe0d0)","linear-gradient(135deg,#f5d5e8,#ffd5c8)",
  ...
  const syms = ["✦","✧","❋","✿","❀","✾","❁","✸","✺","✻"];
  return (
    <motion.div className="polaroid-wrap"
      animate={{ y:[0,-7,0] }} transition={{ duration:4, repeat:Infinity, ease:"easeInOut" }}
      style={{ transform:`rotate(${(page%2===0?1:-1)*(page%3+1)}deg)` }}>
      <div className="polaroid">
        <div className="photo-area" style={{ background:grads[(page-1)%grads.length] }}>
          <img src={src} alt={`Memory ${page}`}
            className="absolute inset-0 w-full h-full object-cover"
            onError={e=>e.target.style.display="none"} />
          <span style={{ fontSize:"clamp(28px,6vh,52px)", color:"rgba(150,80,180,.18)" }}>
            {syms[(page-1)%syms.length]}
          </span>
        </div>
        <p style={{ textAlign:"center", color:"#c4a0d0",
          fontSize:"clamp(9px,1vh,12px)", fontStyle:"italic",
          marginTop:5, letterSpacing:".07em", fontFamily:"Georgia,serif" }}>
          ~ memory {page} ~
        </p>
      </div>
    </motion.div>
    ...
    <div className="fixed inset-0 pointer-events-none overflow-hidden" style={{ zIndex:20 }}>
      {pieces.map(p => (
        <motion.div key={p.id} className="absolute top-0 rounded-sm"
          style={{ left:`${p.x}%`, width:p.sz, height:p.sz*1.4, background:p.color }}
...
const MusicBtn = ({ started }) => {
  ...
  useEffect(() => {
    audio.current = new Audio(`/${CONFIG.repoName}/music/music.mp3`);
  ...
  return (
    <motion.button initial={{ opacity:0,scale:0 }} animate={{ opacity:1,scale:1 }}
      transition={{ delay:.5 }} onClick={toggle}
      className="fixed z-50 flex items-center justify-center rounded-full transition-all"
      style={{ top:12, right:12, width:40, height:40, fontSize:16,
        background:"rgba(196,150,220,.3)", border:"1px solid rgba(196,150,220,.5)",
        backdropFilter:"blur(10px)", color:C.lav600,
...
const CursorGlow = () => {
  ...
  return (
    <div className="fixed pointer-events-none hidden md:block" style={{ zIndex:40,
      left:p.x-160, top:p.y-160, width:320, height:320, borderRadius:"50%",
...
const S_STYLE = {  // shared fixed-screen wrapper
...
const LoadingScreen = ({ onDone }) => {
  ...
  useEffect(() => {
    ...
    const pi = setInterval(() => {
      setProg(p => { if(p>=100){clearInterval(pi);clearInterval(ti);setTimeout(onDone,400);return 100;} return p+1.8; });
    }, 55);
    return () => { clearInterval(ti); clearInterval(pi); };
  }, [onDone]);
  return (
    <div style={{ ...S_STYLE, background:bgMain }}>
      <FloatingPetals count={12} />
      <div style={{ position:"relative", zIndex:1, textAlign:"center" }}>
        <motion.div animate={{ scale:[1,1.1,1] }} transition={{ duration:3, repeat:Infinity }}
          className="float-anim" style={{ fontSize:"clamp(36px,6vh,60px)", marginBottom:"clamp(12px,2vh,24px)" }}>🌸</motion.div>
        <motion.p key={txt} initial={{ opacity:0, y:8 }} animate={{ opacity:1, y:0 }}
          style={{ color:C.textSec, fontSize:"clamp(10px,1.4vh,15px)", letterSpacing:".2em",
            textTransform:"uppercase", marginBottom:"clamp(14px,2.5vh,28px)" }}>
          {txt}
        </motion.p>
        <div style={{ width:"clamp(120px,20vw,180px)", height:2, background:"rgba(196,150,220,.2)",
          borderRadius:100, overflow:"hidden", margin:"0 auto" }}>
          <motion.div style={{ width:`${prog}%`, height:"100%",
...
const WelcomeScreen = ({ onEnter }) => (
  <div style={{ ...S_STYLE, background:bgMain }}>
    ...
    <motion.div initial={{ opacity:0, y:30 }} animate={{ opacity:1, y:0 }} transition={{ duration:.9, delay:.2 }}
      className="glass-card" style={{ position:"relative", zIndex:1 }}>
      <motion.div animate={{ scale:[1,1.12,1] }} transition={{ duration:3, repeat:Infinity }}
        style={{ fontSize:"clamp(28px,5vh,48px)", marginBottom:"clamp(8px,1.5vh,16px)" }}>🌸</motion.div>
      <h1 style={{ fontFamily:"Georgia,serif", fontSize:"clamp(22px,4.5vh,46px)", fontWeight:300,
        color:C.lav800, marginBottom:"clamp(8px,1.5vh,16px)", letterSpacing:".04em" }}>
        {CONFIG.herName ? `Hey, ${CONFIG.herName}` : "Hey You 🌸"}
      </h1>
      <p style={{ fontFamily:"Georgia,serif", fontSize:"clamp(12px,1.8vh,17px)", color:C.textSec,
        lineHeight:1.8, marginBottom:"clamp(14px,2.5vh,28px)" }}>
        I made a small little thing<br/>for someone who deserves<br/>more smiles in life.
      </p>
      <motion.button whileHover={{ scale:1.04, boxShadow:"0 0 22px rgba(196,150,220,.4)" }}
        whileTap={{ scale:.97 }} onClick={onEnter} className="btn" style={{ width:"100%" }}>
        Enter ✦
      </motion.button>
      <p style={{ color:C.textMut, fontSize:"clamp(9px,1.2vh,12px)", marginTop:"clamp(8px,1.5vh,14px)", letterSpacing:".12em" }}>
...
const EmotionalIntro = ({ onNext }) => {
  const lines = [
    { t:"Before we begin,", muted:true }, { t:"I want you to know something.", muted:true }, { t:"" },
    { t:"You matter more than you believe." }, { t:"You've been through more than you've ever admitted." },
    { t:"And you're still here — still kind, still soft." }, { t:"" },
  ...
  return (
    <div style={{ ...S_STYLE, background:bgMain }}>
      ...
      <div style={{ position:"relative", zIndex:1, textAlign:"center", maxWidth:"min(520px,90vw)" }}>
        {lines.map((l,i) => (
          <motion.p key={i} initial={{ opacity:0, y:14 }} animate={{ opacity:1, y:0 }}
            ...
            style={{ fontFamily:"Georgia,serif",
              ...
              lineHeight:1.7, marginBottom: l.t ? "clamp(6px,1.2vh,12px)" : "clamp(2px,0.5vh,6px)" }}>
            {l.t}
          </motion.p>
        ))}
        <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }}
          ...
          <motion.button whileHover={{ scale:1.05 }} whileTap={{ scale:.97 }}
            onClick={onNext} className="btn">Continue →</motion.button>
        </motion.div>
      </div>
    </div>
  );
...
// Timeline — two columns on laptop
const TimelinePage = ({ data, page, total, onNext, onPrev }) => {
  ...
  return (
    <div style={{ ...S_STYLE, background:bgGrads[(page-1)%bgGrads.length] }}>
      <FloatingPetals count={12} />
      {/* Dots */}
      <div className="dots" style={{ position:"relative", zIndex:1 }}>
        {Array.from({ length:total }, (_,i) => (
          <div key={i} className={`dot ${i+1===page?"active":"inactive"}`} />
        ))}
      </div>
      {/* Split grid */}
      <motion.div key={page} initial={{ opacity:0, x:30 }} animate={{ opacity:1, x:0 }}
        exit={{ opacity:0, x:-30 }} transition={{ duration:.45, ease:"easeOut" }}
        className="split-grid" style={{ position:"relative", zIndex:1 }}>
        {/* Photo */}
        <div style={{ display:"flex", justifyContent:"center", alignItems:"center" }}>
          <Polaroid src={data.image} page={page} />
        </div>
        {/* Text */}
        <div className="text-side" style={{ textAlign:"center" }}>
          {data.quote.split("\n").map((line,i) => (
            <motion.p key={i} initial={{ opacity:0, y:12 }} animate={{ opacity:1, y:0 }}
              transition={{ delay:.25+i*.1 }}
              style={{ fontFamily:"Georgia,serif", fontSize:"clamp(14px,2.8vh,22px)",
          ...
          <motion.p initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ delay:.55 }}
            style={{ fontFamily:"Georgia,serif", fontSize:"clamp(10px,1.4vh,14px)",
              fontStyle:"italic", color:"rgba(180,100,140,.72)",
              marginTop:"clamp(4px,0.8vh,10px)", marginBottom:"clamp(8px,1.8vh,20px)" }}>
            {data.subtext}
          </motion.p>
          <div className="nav-row" style={{ display:"flex", gap:8, alignItems:"center",
            justifyContent:"center", flexWrap:"wrap" }}>
            {page > 1 && (
              <motion.button whileHover={{ scale:1.04 }} whileTap={{ scale:.96 }}
                onClick={onPrev} className="btn btn-ghost">← Back</motion.button>
            )}
            <motion.button whileHover={{ scale:1.04, boxShadow:"0 0 20px rgba(196,150,220,.35)" }}
              whileTap={{ scale:.96 }} onClick={onNext} className="btn" style={{ minWidth:120 }}>
              {page === total ? "Continue ✦" : "Next →"}
            </motion.button>
          </div>
          <p style={{ color:C.textMut, fontSize:10, letterSpacing:".18em",
            marginTop:"clamp(4px,0.8vh,10px)", textAlign:"center" }}>
            {page} / {total}
          </p>
        </div>
      </motion.div>
...
const AppreciationPage = ({ onNext }) => (
  <div style={{ ...S_STYLE, background:"linear-gradient(160deg,#fdf2ff,#fff5ef,#ffeae0)" }}>
    ...
    <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ duration:1 }}
      ...
      <motion.div animate={{ scale:[1,1.15,1] }} transition={{ duration:2.5, repeat:Infinity }}
        style={{ fontSize:"clamp(34px,6vh,56px)", marginBottom:"clamp(10px,2vh,22px)", color:"#e08080" }}>♡</motion.div>
      <motion.h2 initial={{ opacity:0, y:24 }} animate={{ opacity:1, y:0 }} transition={{ delay:.4 }}
        style={{ fontFamily:"Georgia,serif", fontSize:"clamp(22px,4vh,42px)", fontWeight:300,
          color:C.lav800, lineHeight:1.2, marginBottom:"clamp(8px,1.5vh,16px)" }}>
        Thank you<br/>for existing.
      </motion.h2>
      <motion.p initial={{ opacity:0, y:14 }} animate={{ opacity:1, y:0 }} transition={{ delay:.9 }}
        style={{ fontFamily:"Georgia,serif", fontSize:"clamp(12px,1.8vh,17px)", color:C.textSec,
          lineHeight:1.75, marginBottom:"clamp(4px,1vh,10px)" }}>
      ...
      <motion.p initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ delay:1.3 }}
        style={{ fontFamily:"Georgia,serif", fontSize:"clamp(10px,1.4vh,14px)", fontStyle:"italic",
          color:"rgba(200,120,140,.7)", marginBottom:"clamp(14px,2.5vh,28px)" }}>
        That kind of heart is rare.
      </motion.p>
      <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ delay:1.8 }}
        style={{ display:"flex", justifyContent:"center" }}>
        <motion.button whileHover={{ scale:1.05, boxShadow:"0 0 22px rgba(255,170,120,.4)" }}
          whileTap={{ scale:.97 }} onClick={onNext} className="btn btn-peach">
          One last thing →
        </motion.button>
      </motion.div>
    </motion.div>
...
const FinalPage = ({ onReplay }) => {
  ...
  return (
    <div style={{ ...S_STYLE, background:"linear-gradient(160deg,#f8eeff,#fff5ee,#ffe8de)", overflow:"hidden" }}>
      ...
      <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ duration:1 }}
        ...
        <div style={{ textAlign:"center" }}>
          <motion.div initial={{ scale:0 }} animate={{ scale:[0,1.3,1] }} transition={{ duration:.8, delay:.2 }}
            style={{ fontSize:"clamp(32px,5.5vh,60px)", marginBottom:"clamp(6px,1.2vh,14px)" }}>🎂</motion.div>
          <motion.h1 initial={{ opacity:0, scale:.85 }} animate={{ opacity:1, scale:1 }} transition={{ delay:.5 }}
            style={{ fontFamily:"Georgia,serif", fontSize:"clamp(22px,4.5vh,46px)", fontWeight:300,
              background:"linear-gradient(135deg,#9060c0,#e07070)", WebkitBackgroundClip:"text",
              WebkitTextFillColor:"transparent", backgroundClip:"text", marginBottom:"clamp(6px,1.2vh,12px)" }}>
            Happy Birthday
          </motion.h1>
          <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ delay:.7 }}
            style={{ fontSize:"clamp(22px,3.5vh,34px)", marginBottom:"clamp(8px,1.5vh,16px)" }}>💖</motion.div>
        ...
        <div style={{ textAlign:"center" }}>
          <motion.div initial={{ opacity:0, y:14 }} animate={{ opacity:1, y:0 }} transition={{ delay:1.4 }}
            className="final-box" style={{ marginBottom:"clamp(10px,2vh,18px)" }}>
            <p style={{ fontFamily:"Georgia,serif", fontSize:"clamp(14px,2.4vh,20px)", color:C.lav800, lineHeight:1.35 }}>
              You deserve happiness.
            </p>
            <p style={{ fontFamily:"Georgia,serif", fontSize:"clamp(10px,1.3vh,14px)", fontStyle:"italic",
              color:"rgba(180,120,160,.65)", marginTop:6 }}>
              Never forget that.
            </p>
          </motion.div>
          <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ delay:2 }}
            style={{ display:"flex", flexDirection:"column", gap:8, alignItems:"center" }}>
            <button onClick={() => setConfetti(true)} className="btn btn-ghost"
              style={{ width:"100%", maxWidth:240 }}>🎉 More confetti!</button>
            <button onClick={onReplay} className="btn" style={{ maxWidth:240, width:"100%" }}>↺ Replay Journey</button>
          </motion.div>
          <p style={{ color:C.textMut, fontSize:"clamp(9px,1.2vh,12px)", letterSpacing:".14em", marginTop:"clamp(8px,1.5vh,16px)" }}>
  ...
  return (
    <div style={{ position:"fixed", inset:0, overflow:"hidden" }}>
      ...
      <AnimatePresence mode="wait">
        {sc===SC.LOAD && (
          <motion.div key="l" exit={{ opacity:0 }} transition={{ duration:.5 }} style={{ position:"fixed", inset:0 }}>
            <LoadingScreen onDone={() => go(SC.WELCOME)} />
          </motion.div>
        )}
        {sc===SC.WELCOME && (
          <motion.div key="w" initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }} transition={{ duration:.8 }} style={{ position:"fixed", inset:0 }}>
            <WelcomeScreen onEnter={() => { setMu(true); go(SC.INTRO); }} />
          </motion.div>
        )}
        {sc===SC.INTRO && (
          <motion.div key="i" initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }} transition={{ duration:.8 }} style={{ position:"fixed", inset:0 }}>
            <EmotionalIntro onNext={() => go(SC.TIMELINE)} />
          </motion.div>
        )}
        {sc===SC.TIMELINE && (
          <motion.div key={`t${pg}`} initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }} transition={{ duration:.45 }} style={{ position:"fixed", inset:0 }}>
        ...
        {sc===SC.APPREC && (
          <motion.div key="a" initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }} transition={{ duration:.8 }} style={{ position:"fixed", inset:0 }}>
            <AppreciationPage onNext={() => go(SC.FINAL)} />
          </motion.div>
        )}
        {sc===SC.FINAL && (
          <motion.div key="f" initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }} transition={{ duration:.8 }} style={{ position:"fixed", inset:0 }}>
